...
the background worker that turns design jobs into manufactured artifacts. the engine (`engines/circuitron/`) is pure and synchronous — this daemon is what makes it a service: claim a job, run the engine, retry with adjusted parameters when routing struggles, persist the outputs.
