...
"generative openscad" for keeberia means **deterministic codegen of parametric scripts** — not llm-authored geometry, not text-to-cad, not rodin-style multimodal mesh generation. the case compiler already proves the shape of it: `backend/cad/case-engine` emits **vanilla, dependency-free, parametric openscad** where every knob the app exposes is a top-level variable ("these are the app's sliders"), board coordinates match the pcb's, and re-rendering with different params is the whole point. traceparency extends to geometry: same project model → same .scad, today and in two years.
