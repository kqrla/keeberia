...
export function generateCase(pcb: PcbResult, partial?: Partial<CaseOptions>): CaseResult {
  ...
  L.push(`// (vanilla openscad, no libraries). coordinates match the pcb's board`);
  L.push(`// coordinates: (0,0) is the board center, +y is toward the usb wall.`);
  L.push(``);
  L.push(`// ── parameters (these are the app's sliders) ─────────────────`);
  ...
  L.push(`case_margin    = ${fmt(opts.caseMargin)};     // pcb edge to inner wall (print tolerance)`);
  L.push(`wall_thickness = ${fmt(opts.wallThickness)};     // side walls`);
  L.push(`base_thickness = ${fmt(opts.baseThickness)};     // case floor`);
  L.push(`corner_radius   = ${fmt(opts.cornerRadius)};     // outer corner radius`);
  L.push(`front_height   = ${fmt(opts.frontHeight)};     // wall height, usb edge`);
  L.push(`rear_height    = ${fmt(opts.rearHeight)};     // wall height, far edge`);
  L.push(`standoff_height = ${fmt(opts.standoffHeight)};     // pcb floats this far above the floor`);
  L.push(`screw_size     = ${fmt(opts.screwSize)};    // m2 drill`);
  L.push(`plate_thickness = ${fmt(opts.plateThickness)};    // switch plate (mx 1.5, choc 1.2)`);
  L.push(`$fn = 48;`);
  L.push(``);
