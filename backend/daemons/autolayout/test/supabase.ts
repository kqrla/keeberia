...
// (from backend/daemons/autolayout). exits 0 only if every step passes.
...
async function main() {
  ...
  if (!probe.ok && (await probe.text()).includes("PGRST205")) {
    console.log("table missing — apply backend/supabase/migrations/0001_design_jobs.sql in the dashboard sql editor first");
    process.exit(1);
  }
